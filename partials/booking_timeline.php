<?php
/**
 * Logistics booking timeline: recorded events first, then the milestones still
 * to come so the customer can see the whole journey.
 * @var array $booking @var array $events
 */
...
$currentIndex = array_search($booking['status'], $milestones, true);
...
<ul class="timeline">
    <?php foreach ($events as $event): ?>
        <li class="done">
            <span class="dot"></span>
            <div class="t"><?= e(Booking::statuses()[$event['status']] ?? ucfirst($event['status'])) ?></div>
    ...
    <?php if (!$cancelled): ?>
        <?php foreach ($milestones as $i => $milestone): ?>
            ...
            <li class="pending">
                <span class="dot"></span>
                <div class="t"><?= e(Booking::statuses()[$milestone] ?? ucfirst($milestone)) ?></div>
